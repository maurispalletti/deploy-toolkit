...
function looksLikeNoWebAppsError(stderr) {
  // Firebase CLI phrasing varies between versions. We keep a small set
  // of substrings that cover the cases we've seen in the wild:
  //   - "No matching apps"
  //   - "No apps found"
  //   - "no web app" (case-insensitive)
  //   - "Need to register"
  // The CLI exits non-zero in all of these.
...
function looksLikeProjectMissing(stderr) {
  const s = stderr.toLowerCase();
  return (
...
export async function fetchWebSdkConfig(projectId, opts = {}) {
  ...
  if (looksLikeProjectMissing(result.stderr)) {
    const e = new Error(`Firebase project not found: ${projectId}`);
    e.code = "PROJECT_NOT_FOUND";
    throw e;
  }
  if (looksLikeNoWebAppsError(result.stderr)) {
