...
test("fetchWebSdkConfig: retry-after-create when CLI returns the no-web-apps error as JSON on stdout", async () => {
  // Modern firebase CLI in --json mode reports errors as JSON on STDOUT,
  // not stderr. The retry-after-create logic must inspect both streams.
  const errPayload = JSON.stringify({
    status: "error",
    error: "There are no WEB apps associated with this Firebase project"
  });
  const { exec, calls } = makeExec([
    { stdout: errPayload, stderr: "", code: 1 },
    { stdout: "App created.", stderr: "", code: 0 },
    { stdout: JSON.stringify(HAPPY_PAYLOAD), stderr: "", code: 0 }
  ]);
  const config = await fetchWebSdkConfig("my-app-k3p9", { exec });
  assert.equal(config.projectId, "my-app-k3p9");
  assert.equal(calls.length, 3, "should have retried after creating a Web App");
});
