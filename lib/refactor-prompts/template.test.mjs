...
test("generateAuthRefactorPrompt: vite-react with pre-fetched SDK config", () => {
  const md = generateAuthRefactorPrompt({
    appName: "notes",
    framework: "vite-react",
    projectId: "notes-k3p9",
    sdkConfig: {
      apiKey: "AIza-fake",
      authDomain: "notes-k3p9.firebaseapp.com",
      projectId: "notes-k3p9",
      storageBucket: "notes-k3p9.appspot.com",
      messagingSenderId: "1",
      appId: "1:1:web:x"
    },
    scaffoldedConfigPath: "src/firebase-config.js"
  });
  // Title is the A1-specific phrasing.
  assert.match(md, /^# Refactor for Firebase: add Google sign-in to your app/m);
  // App name and framework appear.
  assert.match(md, /`notes`/);
  assert.match(md, /`vite-react`/);
  // The "what we detected" block calls out the wizard-written config path.
  assert.match(md, /already written to `src\/firebase-config\.js`/);
  // The project ID surfaces somewhere.
  assert.match(md, /notes-k3p9/);
  // The recipe mentions firebase install + signInWithPopup.
  assert.match(md, /npm install firebase/);
  assert.match(md, /signInWithPopup/);
  assert.match(md, /onAuthStateChanged/);
  // Reference to the template implementation is preserved.
  assert.match(md, /templates\/auth\/vite-react\/SignInWithGoogle\.jsx/);
  // Re-run instruction.
  assert.match(md, /```bash\n\.\/deploy-app\n```/);
});

test("generateAuthRefactorPrompt: unknown framework + no SDK config", () => {
  // When the SDK-config fetch failed mid-wizard (sdkConfig null), the
  // prompt acknowledges that and tells the AI how to fill in the gap.
  const md = generateAuthRefactorPrompt({
    appName: "demo",
    framework: "unknown",
    projectId: "demo-xxxx",
  ...
  // Title still renders.
  assert.match(md, /^# Refactor for Firebase: add Google sign-in to your app/m);
  // The detected block reports the missing-config state.
  assert.match(md, /NOT pre-fetched/);
  // We default to a sensible path even when nothing was scaffolded.
  assert.match(md, /firebase-config\.js/);
  // The Firebase install step is still in the steps list.
  assert.match(md, /Add `firebase` to your app's `package\.json`/);
});

test("generateAuthRefactorPrompt: nextjs picks the lib/ config path by default", () => {
  const md = generateAuthRefactorPrompt({
    appName: "shop",
    framework: "nextjs",
    projectId: "shop-1",
    sdkConfig: {
      apiKey: "AIza-x", authDomain: "shop-1.firebaseapp.com", projectId: "shop-1",
      storageBucket: "shop-1.appspot.com", messagingSenderId: "2", appId: "1:2:web:y"
    },
    scaffoldedConfigPath: "lib/firebase-config.js"
  });
  assert.match(md, /`nextjs`/);
  assert.match(md, /lib\/firebase-config\.js/);
  // Next.js entry-component hint references the App Router OR Pages Router.
  assert.match(md, /app\/layout\.tsx/);
});
