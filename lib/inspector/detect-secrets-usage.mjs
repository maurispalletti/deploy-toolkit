...
const SECRET_PATTERNS = [
  ...
  // Google API key — at least 35 chars after `AIza`. Real keys are
  // exactly 35; we use `{35,}` so the regex still finds the value when
  // the surrounding token is longer (test fixtures, padding, etc.).
  // Surfaced with a dedicated kind so the UI can render this as
  // low-urgency (Firebase SDK keys legitimately use this prefix).
