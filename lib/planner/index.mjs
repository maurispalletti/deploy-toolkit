...
export function plan(inspection, answers) {
  // When the folder name matched an existing Firebase project, use the name
  // as-is. Otherwise append a random suffix so the new project ID is unique.
  const projectId = answers.existingProject
    ? slugify(answers.appName)
    : `${slugify(answers.appName)}-${shortSuffix()}`;
